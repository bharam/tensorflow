...
///
/// - snapmax(size: INT) -> INT
/// - snapmax(size: NULL|REAL|TEXT|BLOB) -> SQLITE_MISMATCH
///
///   Returns the maximum number of bytes a BLOB returned by snap() can be,
///   given the byte length of the data it receives. At the time of this
///   writing, Snappy defined that as 32 + size + size/6. This function can
///   be useful in conjunction with zeroblob().
///
...
#include "sqlite/src/sqlite3ext.h"
...
static void snapmax(sqlite3_context* ctx, int /*argc*/, sqlite3_value** argv) {
  if (sqlite3_value_type(argv[0]) != SQLITE_INTEGER) {
    sqlite3_result_error(ctx, "snapmax() needs int", -1);
    sqlite3_result_error_code(ctx, SQLITE_MISMATCH);
    return;
  }
  int64 size = sqlite3_value_int64(argv[0]);
  if (size < 0) {
  ...
  sqlite3_result_int64(ctx, static_cast<int64>(output_size));
}
